...
def twoq_pulse_distort_err(a, fi, fj, ac_spectrum_paras1, ac_spectrum_paras2):
    ...
    if np.isnan(vi0) or np.isnan(vi1) or np.isnan(vj0) or np.isnan(vj1):
        return 5e-4
    else:
        return a[0] * (abs(vi0 - vi1) + abs(vj0 - vj1)) + inner_leakage(a[1:], fi, fj)


def err_model(frequencys=None, xtalk_graph=None, a=None, targets=None, isTrain=False, freqIndex=True):
    ...
    if frequencys is None:
        ...
        for node in targets:
            xtalk_graph.nodes[node]['frequency'] = frequencys[targets.index(node)]
            if xtalk_graph.nodes[node]['frequency'] < np.min(xtalk_graph.nodes[node]['allow freq']) - 1:
                print(node, xtalk_graph.nodes[node]['frequency'], '<', np.min(xtalk_graph.nodes[node]['allow freq']))
            elif xtalk_graph.nodes[node]['frequency'] > np.max(xtalk_graph.nodes[node]['allow freq']) + 1:
                print(node, xtalk_graph.nodes[node]['frequency'], '>', np.max(xtalk_graph.nodes[node]['allow freq']))
    ...
    for target in targets:
        ...
        if isinstance(target, str):
            ...
            fWork = max(xtalk_graph.nodes[target]['frequency'], np.min(xtalk_graph.nodes[target]['allow freq']))
            pulseql = fWork
...
def err_model_fun(a, frequencys, xtalk_graph, errors=None, returnLoss=True):
    ...
    if returnLoss:
        ...
        print(np.mean(np.abs(errors - pred_errs) * np.where(errors != 0, 1, 0)))
        return np.mean(np.abs(errors - pred_errs) * np.where(errors != 0, 1, 0))
    else:
        return pred_errs
...
def train_a_model(data_loader, xtalk_graph):
    ...
    for batch_data in data_loader:
        ...
        real_err = batch_data[1].numpy()
                
        # 设置边界，确保所有参数大于等于零
        bounds = [(0, None) for _ in range(len(aIni))]

        F = minimize(err_model_fun, aIni, args=(frequency, xtalk_graph, real_err), bounds=bounds, method='cg',  options={'maxiter': 50})
    ...
    # 创建一个布尔索引，选择 B 中非零元素的位置
    non_zero_indices = b_i != 0
    # 使用布尔索引过滤 A 和 B
    a_i = a_i[non_zero_indices]
    b_i = b_i[non_zero_indices]

    min_val = np.min([np.min(a_i), np.min(b_i)])
    max_val = np.max([np.max(a_i), np.max(b_i)])
    ...
    # 添加标题和标签
    plt.title('train relav', fontsize=15)
    plt.semilogx()
    plt.xlabel('relav inacc', fontsize=15)
    ...
    # 添加标题和标签
    plt.title('train abs', fontsize=15)
    plt.semilogx()
    plt.xlabel('inacc', fontsize=15)
    plt.ylabel('cdf', fontsize=15)
    plt.legend(fontsize=15)
...
def test_a_model(a, data_loader, xtalk_graph):
    ...
    plt.title('test', fontsize=15)
    plt.semilogx()
    plt.xlabel('relev inacc', fontsize=15)
    plt.ylabel('cdf', fontsize=15)
    plt.legend(fontsize=15)
    ...
    # 添加标题和标签
    plt.title('test abs', fontsize=15)
    plt.semilogx()
    plt.xlabel('inacc', fontsize=15)
    plt.ylabel('cdf', fontsize=15)
    plt.legend(fontsize=15)

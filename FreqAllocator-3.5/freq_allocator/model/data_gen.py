...
class MyDataset(Dataset):
    # 初始化方法，传入原始数据和标签
    def __init__(self, H, W, chip, data=None, labels=None, ifFake=False, is_a=False):
        self.H = H
        self.W = W
        self.chip = chip
        self.xtalk_graph = self.gen_xtalk_graph(chip)
        if ifFake:
            ...
            if is_a:
                ...
                for node in self.xtalk_graph.nodes:
                    nodeId = list(self.xtalk_graph.nodes).index(node)
                    assert x[:, nodeId].min() >= self.xtalk_graph.nodes[node]['freq_min']
                    assert x[:, nodeId].max() <= self.xtalk_graph.nodes[node]['freq_max']
    ...
    def __generate_fake_data__(self):
        ...
        # 计算 x 数组的值
        x[:, :] = allow_freq_array[np.arange(len(self.xtalk_graph.nodes)), frequencysid]

        # for node in self.xtalk_graph.nodes:
        #     nodeId = list(self.xtalk_graph.nodes).index(node)
        #     assert np.min(x[:, nodeId]) >= self.xtalk_graph.nodes[node]['freq_min']
        #     assert np.max(x[:, nodeId]) <= self.xtalk_graph.nodes[node]['freq_max']
        ...
        freqIndex = [False] * len(frequencysid) 
        p = Pool(cpu_count())
        # err1 = p.starmap(freq_allocator.model.err_model, zip(frequencysid, xtalk_graphs, aes, targets, isTrain))
        err = p.starmap(freq_allocator.model.err_model, zip(x, xtalk_graphs, aes, targets, isTrain, freqIndex))
        p.close()
        p.join()
    ...
    def gen_xtalk_graph(cls, chip):
        ...
        for qcq in chip.edges():
            ...
            if chip.nodes[qh]['freq_min'] + chip.nodes[qh]['anharm'] > chip.nodes[ql]['freq_max'] or \
                ...
                two_qubit_graph.nodes[qcq]['freq_max'] = ub
                two_qubit_graph.nodes[qcq]['freq_min'] = lb

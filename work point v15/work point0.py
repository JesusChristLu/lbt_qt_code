...
def energy_diff(omegac, g_qc, g_qq, energyLevel, bitNum, omegaq, anharm, state):
    H0, Hi = H(g_qc, g_qq, omegaq, omegac, bitNum, energyLevel, anharm)
    _, _, energy, energy0 = eigensolve(H0, H0 + Hi, energyLevel, bitNum)
    res = []
    for s in state:
        res.append(np.abs(energy[state_number_index([energyLevel] * bitNum, s)] - \
            energy0[state_number_index([energyLevel] * bitNum, s)]))
    return res
...
def spectator_CZ(tg, g_qc, g_qq, freqqMax, freqcMax, bitNum, energyLevel, anharm, sq, lambdas=None):
    ...
    res = minimize(evolution_CZ, lambdasIni, bounds=bounds, args=(tg, g_qc, g_qq, freqqMax, freqcMax, 3, energyLevel, anharm, gq, 0),
...
if __name__ == '__main__':
    ...
    anharm = {'q0' : -240 * 1e-3, 'c0' : -200 * 1e-3, 'q1' : -150 * 1e-3, 'c1' : -200 * 1e-3, 'q2' : -200 * 1e-3}
    C_q = [97, 99, 97]
    C_c = [95, 100]
    ...
    # res = minimize(energy_diff, omegac, 
    # args=(g_qc, g_qq, 3, 3, (omega0, omega1), anharm, ([1, 0, 1], [2, 0, 0])),
    # method='Nelder-Mead', options={'ftol' : 1e-9})
    # omegac = res.x[0]
    # print(res.fun)
    ...
    qstep = 500
    cstep = 1
    omegasList = np.arange(qlow, qhigh, (qhigh - qlow) / qstep)
    omegacList = np.arange(clow, chigh, (chigh - clow) / cstep)
    bitNum = 3
    energyLevel = 4

    omega0s = [omega0] * len(omegasList) * len(omegacList)
    g_qcs = [g_qc] * len(omegasList) * len(omegacList)
    bitNums = [bitNum] * len(omegasList) * len(omegacList)
    energyLevels = [energyLevel] * len(omegasList) * len(omegacList)
    anharms = [anharm] * len(omegasList) * len(omegacList)
    g_qqs = [g_qq] * len(omegasList) * len(omegacList)
    states = [[[0, 0, 1], [0, 0, 2], [1, 0, 1], [1, 0, 2]]] * len(omegasList) * len(omegacList)
    states0 = [[[0, 0, 1, 0, 1], [0, 0, 2, 0, 0]]] * len(omegasList) * len(omegacList)
    states1 = [[[1, 0, 1, 0, 1], [1, 0, 2, 0, 0]]] * len(omegasList) * len(omegacList)
    omegass = []
    omegacs = []
    for s in omegasList:
        for c in omegacList:
            omegass.append([omega0, s])
            omegacs.append([c])
            # omegass.append([omega0, omega1, s])
            # omegacs.append([omegac, c])

    tStart = time.time()
    print(cpu_count())
    p = Pool(cpu_count())
    xi = p.starmap(energy_diff, zip(omegacs, g_qcs, g_qqs, energyLevels, bitNums, omegass, anharms, states))
    p.close()
    p.join()
    t = time.time() - tStart
    print(t)
    ...
    ax = plt.subplot(211)
    ax.plot((omegasList), xi01, label='01')
    ax.plot((omegasList), xi02, label='02')
    ax.plot((omegasList), np.array(xi02) - np.array(xi01), label='02-01')
    plt.legend()
    ax = plt.subplot(212)
    ax.plot((omegasList), xi11, label='11')
    ax.plot((omegasList), xi12, label='12')
    ax.plot((omegasList), np.array(xi12) - np.array(xi11), label='12-11')
    plt.legend()
    plt.show()
    # draw_heat_map(omegasList, omegacList, delta0, 'E_00101- E_00200', 'E_00101- E_00200', 'omegas(GHz)', 'omegac(GHz)', 'abs')
    # draw_heat_map(omegasList, omegacList, delta1, 'E_10101- E_10200', 'E_10101- E_10200', 'omegas(GHz)', 'omegac(GHz)', 'abs')

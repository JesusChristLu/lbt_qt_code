...
class Get_alg():
    ...
    def matrix_to_graph(self, mat): 
        ...
        if self.show:
            ...
            plt.savefig("vertexrankdemo2.pdf", dpi = 300)
            plt.show()

            plt.ion()
            labels_params = {"font_size":30}
            pos = nx.circular_layout(G)
            nx.draw(G, pos, **labels_params, node_color='y', edgelist=edges, width=weights, edge_color='red', with_labels=True, node_size = 2000)
            plt.savefig("vertexrankdemo.pdf", dpi = 300)
            plt.draw()
            plt.pause(10)
            plt.close()

...
class Split():
    def __init__(self, g, origing, prog, recover, show=False, a = 0.5):
        self.show = show
    ...
    def split_vertex(self, a, splitVertexes, recover, g):
        ...
        if self.show:
            ...
            plt.savefig("weightallocdemo4.pdf", dpi = 300)
            plt.pause(5)
            plt.close()
        ...
        if self.show:
            ...
            labels_params = {"font_size":20}
            # nx.draw(graph, **labels_params, with_labels=True, node_color=node_color, font_color='white', node_size = 1000, width=5)
            nx.draw(graph, **labels_params, pos=pos, with_labels=True, node_color=node_color, font_color='white', node_size = 1000, width=5)
            plt.savefig("afterrecover.pdf", dpi = 300)
            plt.pause(50)
            plt.close()
        return graph, layout
    ...
    def choose_media_structure(self, a, v, graph, recover):
        ...
        while True:
            ...
            if (len(bestAllocScores) >= 2 and list(bestAllocScores[-1].keys())[0] >= list(bestAllocScores[-2].keys())[0]):
                oldBestA = list(bestAllocScores[-2].values())[0]
                break
            elif (len(bestAllocScores) >= 1 and n >= 7):
                oldBestA = list(bestAllocScores[-1].values())[0]
                oldBestS = bestS
                break
            else:
                n += 1
    ...
    def get_I(self, a, prog, v, graph, recover):
        ...
        if self.show:
            plt.ion()
            sns.heatmap(I, annot = True, cmap='Reds', annot_kws={'size':15})
            cax = plt.gcf().axes[-1]
            cax.tick_params(labelsize=14)
            plt.xticks(fontsize=14)
            plt.yticks(fontsize=14)
            plt.savefig("weightallocdemo1.pdf", dpi = 300)
            plt.show()
            plt.pause(5)
            plt.close()     

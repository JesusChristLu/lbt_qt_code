    ...
    for qubit in qubitList:
        if qubit in inducedChip.nodes:
            ...
            if qubit in tq or qubit[::-1] in tq:
                freqqWork = max(inducedChip.nodes[qubit[0]]['freq'], inducedChip.nodes[qubit[1]]['freq']) + anharmq
...
def zzcoupling(inducedChip, tq, rho_qq, rho_qc, energyLevel, anharmq, anharmc, ada=False):
    ...
    if ada:
        ...
        order00 = subStatesId[state_number_index([energyLevel] * qubitNum, s00)]
...
if __name__ == '__main__':
    ...
    # freq2 = 4.1
    # freq1 = freq2 + offDetune
    # # freq1 = freq2 + nearOnDetune
    # # freq1 = freq2 + onDetune

    # print(round(freq1, 2), round(freq2, 2))

    # cFreqLow = max([freq1, freq2]) + 0.5
    # cFreqHigh = 7
    # # cFreqLow = 3
    # # cFreqHigh = min([freq1, freq2]) - 0.5
    ...
    # write_data(str(round(freq1, 2)) + ' ' + str(round(freq2, 2)) + ' shift11 p.txt', shift11)
    # # write_data(str(round(freq1, 2)) + ' ' + str(round(freq2, 2)) + ' shift11 n.txt', shift11)
    ...
    #     res = minimize(second_object, [7], args=(chip1, rho_qq, rho_qc, energyLevel, anharmq, anharmc, 0), 
    ...
    #     res2 = minimize(second_object, [7], args=(chip2, rho_qq, rho_qc, energyLevel, anharmq, anharmc, 0), 
    #         bounds=((5, 8),), method='Powell', options={'maxiter' : 50})
    ...
    # # write_data('flat gaussian two Q para cq2.txt', calibratedTwoPara)
    # write_data('flat gaussian two Q para noncq2.txt', calibratedTwoPara)
    ...
    # lambdas = [6.17111824, 1.0187895,
    #            5.08267416, 1.01077973, 4.35227526, 0.97687807,
    #            6.16413116, 0.9931174,
    #            4.70050448, 1.00765324, 4.38896186, 1.05862603]

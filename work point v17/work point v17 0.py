...
def gaus_flat(tList, pulseLen, sigma, freqWork, freqMax):
    ...
    # x_idle = paras_dict['w_idle']
    # x_work = paras_dict['w_work']
    # tg = paras_dict['t_g']
    # tw = paras_dict.get('tw', 5)  # idle 时间，默认为 5 ns
    # overshoot_amp = np.abs(x_idle - x_work) ** 2 * 1.5
    # pulse_len = tg - tw * 2 - sigma * 2
    # print(overshoot_amp)

    # # 上升沿和下降沿的位置
    # flattop_start = tw + 3 * sigma
    # flattop_end = tw + pulse_len - 3 * sigma

    # # 初始化波形
    # y_list = np.full_like(t_list, x_idle)

    # # 主波形部分
    # main_waveform = (x_work - x_idle) * 0.5 * (
    #     erf((t_list - flattop_start) / (np.sqrt(2) * sigma)) -
    #     erf((t_list - flattop_end) / (np.sqrt(2) * sigma))
    # ) + x_idle

    # # 过冲修正
    # overshoot = np.zeros_like(t_list)
    # if x_idle > x_work:
    #     # 上升沿过冲
    #     overshoot -= overshoot_amp * np.exp(-(t_list - flattop_start) ** 2 / (2 * sigma ** 2))
    #     # 下降沿过冲
    #     overshoot += overshoot_amp * np.exp(-(t_list - flattop_end) ** 2 / (2 * sigma ** 2))
    # else:
    #     # 上升沿过冲
    #     overshoot += overshoot_amp * np.exp(-(t_list - flattop_start) ** 2 / (2 * sigma ** 2))
    #     # 下降沿过冲
    ...
    # # 在 tw 时间之前保持 idle 水平
    # active_region = (t_list >= tw) & (t_list <= tg - tw)
    # y_list[active_region] = main_waveform[active_region] + overshoot[active_region]

    # # tw 时间结束后保持 idle 水平
    # y_list[t_list > tw + pulse_len] = x_idle
...
if __name__ == '__main__':
    ...
    delta12 = omega0 - omega1List
    draw_heat_map(delta12, omegacList, zeta_list, 'zz', 'zz', 'detune12(MHz)', 'omegac(GHz)', 'logabs', threshold=-1.3)# 1.1 0.7
    ...
    # [4.3068077  5.80206255 0.83374141 0.80064436] 3.3956483153962935e-05
    ...
    # omega0, omega1 = 4.5, 4.1
    # omegac = 7
    # _, g_off, _, _, _ = zzcoupling(g_qc, g_qq, energyLevel, bitNum, [omega0, omega1], [omegac], anharm)
    # print(g_off) # -0.002141875191517477
    # # 共振情况
    # omega0, omega1 = 4.32, 4.1
    # omegac = 5
    # _, g_on, _, _, _ = zzcoupling(g_qc, g_qq, energyLevel, bitNum, [omega0, omega1], [omegac], anharm)
    # print(g_on) # -0.025561862267751412

    # 画两笔特门时序信号
    # tg = 60
    # t = np.arange(0, tg, tg / 300)
    # sigmaq = 0.83374141
    # sigmac = 0.80064436
    # freqqWork = 4.3068077
    # freqqMax = 4.5
    # freqcWork = 5.80206255
    # freqcMax = 7
    # freqqList = gaus_flat(t, tg, sigmaq, freqqWork, freqqMax)
    # freqcList = gaus_flat(t, tg, sigmac, freqcWork, freqcMax)
    # plt.plot(t, freqqList, label='omega1 pulse')
    # plt.plot(t, freqcList, label='omegac pulse')

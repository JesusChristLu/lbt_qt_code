...
class NoiseAdaptiveLayout(AnalysisPass):
    ...
    def _select_best_remaining_cx(self):
        ...
        for item in candidates:
            if self.gate_reliability[item] > best_reliab:
                ...
                best_item = item
        if best_item == None:
            print('There is no directly connected gate for these two qubits. !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            best_item = tuple(sorted([idx for idx in self.available_hw_qubits], key=lambda x : self.readout_reliability[x], reverse=True)[:2])
        return best_item
    ...
    def run(self, dag):
        ...
        while self.pending_program_edges:
            ...
            q1_mapped = edge[0] in self.prog2hw
            q2_mapped = edge[1] in self.prog2hw
            # print(edge[:2], (q1_mapped, q2_mapped))

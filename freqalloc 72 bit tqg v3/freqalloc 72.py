...
def checkcoli(chip, a):
    ...
    for qubit in chip.nodes:
        if chip.nodes[qubit].get('frequency', False) and chip.nodes[qubit]['available']:
            ...
            if badFreqCost / (a[0] * chip.nodes[qubit]['sing tq']) > 1e-4:
            ...
            for neighbor in chip[qubit]:
                ...
                for nextNeighbor in chip[neighbor]:
                    if not(nextNeighbor == qubit) and not((qubit, nextNeighbor) in conflictEdge) and not((nextNeighbor, qubit) in conflictEdge) and chip.nodes[nextNeighbor].get('frequency'):
                        ...
                        if (neighborCost5 < 0.02 or neighborCost6 < 0.02 or neighborCost7 < 0.02):
        ...
        if xtalkG.nodes[qcq].get('frequency', False):
            if sum(qcq[0]) % 2:
                ...
                T2Cost2 = twoq_T2_err(xtalkG.nodes[qcq]['frequency'], chip.nodes[qcq[1]]['frequency'], chip.nodes[qcq[1]]['sweet point'],
                                    a[12], xtalkG.nodes[qcq]['two tq'], chip.nodes[qcq[1]]['df/dphi'])
            if T1Cost1 / (a[11] * xtalkG.nodes[qcq]['two tq']) > 1e-4 or T1Cost2 / (a[11] * xtalkG.nodes[qcq]['two tq']) > 1e-4:
            ...
            for q in qcq:
                for neighbor in chip[q]:
                    if neighbor in qcq:
                        continue
                    neighborCost1 = np.abs(xtalkG.nodes[qcq]['frequency'] - chip.nodes[neighbor]['frequency'])
                    neighborCost2 = np.abs(xtalkG.nodes[qcq]['frequency'] - (chip.nodes[neighbor]['frequency'] + chip.nodes[neighbor]['anharm']))
                    if neighborCost1 < 0.04 or neighborCost2 < 0.04:
                    ...
                    for q0 in qcq:
                        for q1 in neighbor:
                            if (q0, q1) in chip.edges:
                                neighborCost3 = np.abs([xtalkG.nodes[qcq]['frequency'] - chip.nodes[q0]['frequency']])
                                if sum(q0) % 2:
                                    neighborCost4 = np.abs(xtalkG.nodes[qcq]['frequency'] + chip.nodes[q0]['anharm'] - xtalkG.nodes[neighbor]['frequency'])
                                    neighborCost5 = np.abs(xtalkG.nodes[qcq]['frequency'] - (xtalkG.nodes[neighbor]['frequency'] - chip.nodes[q1]['anharm']))
                                else:
                                    neighborCost4 = np.abs(xtalkG.nodes[qcq]['frequency'] - chip.nodes[q0]['anharm'] - xtalkG.nodes[neighbor]['frequency'])
                                    neighborCost5 = np.abs(xtalkG.nodes[qcq]['frequency'] - (xtalkG.nodes[neighbor]['frequency'] + chip.nodes[q1]['anharm']))
                    if neighborCost3 < 0.04 or neighborCost4 < 0.04 or neighborCost5 < 0.04:
                        conflictEdge.append((qcq, neighbor))
                        if not(qcq in reOptimizeQCQs):
...
def T1_spectra(fMax, step): # 在实验中，这里用真实的T1谱 ###############################
...
def T2_spectra(fMax, step): # 在实验中，这里用真实的T2谱 ###############################
        ...
        for neighbor in xtalkG[qcq]:
            if xtalkG.nodes[neighbor].get('frequency', False):
                for q0 in qcq:
                    for q1 in neighbor:
                        if (q0, q1) in chip.edges:
                            cost += twoq_xtalk_err([xtalkG.nodes[qcq]['frequency'], chip.nodes[q0]['frequency']], [xtalkG.nodes[neighbor]['frequency'], chip.nodes[q1]['frequency']], a[17], a[18], xtalkG.nodes[qcq]['two tq'])
                            if sum(q0) % 2:
                                cost += twoq_xtalk_err([xtalkG.nodes[qcq]['frequency'] + chip.nodes[q0]['anharm'], chip.nodes[q0]['frequency'] + chip.nodes[q0]['anharm']], 
                                                       [xtalkG.nodes[neighbor]['frequency'], chip.nodes[q1]['frequency']], a[19], a[20], xtalkG.nodes[qcq]['two tq'])
...
def single_gate(a, inchip, last=False):
    chip = copy.deepcopy(inchip)
    ...
    print(a, conflictPercents[-1])
    if last:
        return conflictNodeDict, conflictEdge, twoQForbiddenCoupler, chip
    else:
        return conflictPercents[-1]

def twoq_gate(a, conflictNodeDict, conflictEdge, twoQForbiddenCoupler):
    xTalkG = xtalk_G(chip)
...
if __name__ == '__main__':
    ...
    # a = [0.1, 0.1, 10, 0.04, 10, 0.04, 6, 10, 0.012, 10, 0.012, 
        #  0.1, 0.1, 10, 0.16, 10, 0.16, 10, 0.16, 10, 0.16]
    
    # a = [0.1, 0.1, 10, 0.015, 10, 0.015, 6, 10, 0.015, 10, 0.015, 
        #  0.1, 0.1, 10, 0.16, 10, 0.16, 10, 0.16, 10, 0.16]


    a = [0.00, 0.00, 10, 0.04, 10, 0.04, 6, 10, 0.01, 10, 0.01, 
            0.1, 0.1, 10, 0.16, 10, 0.16, 10, 0.16, 10, 0.16]
    ...
    bounds = [(1e-4, np.inf), (1e-4, np.inf), 
              (1e-4, np.inf), (1e-4, 1), 
              (1e-4, np.inf), (1e-4, 1), 
              (1e-4, np.inf), 
              (1e-4, np.inf), (1e-4, 1), 
    ...
    res = minimize(single_gate, asing, args=(chip,), method='powell', bounds=bounds, options={'maxiter' : 200})
    print(res.x, res.fun)
    ...
    # fList = np.linspace(-0.2, 0.2, 100)
    # plt.plot(fList, lorentzain(fList, 0, 1000, 0.012))
    # plt.show()
...
def cost_function(frequencys, inducedChip, targets, a):
        ...
        for neighbor in chip[target]:
            if chip.nodes[neighbor].get('frequency', False):
                ...
                if chip.nodes[target]['available'] and chip.nodes[neighbor]['available']:
                    cost += 100 * twoQGate_constraint(chip.nodes[neighbor]['frequency'], chip.nodes[target]['frequency'])
...
def twoQ_cost_function(frequencys, chip, xTalkG, reOptimizeQCQs, a):
    ...
    for qcq in reOptimizeQCQs:
        ...
        for neighbor in xtalkG[qcq]:
            if xtalkG.nodes[neighbor].get('frequency', False):
                cost += 2 * twoQ_int_constraint(chip, xtalkG, qcq, neighbor)
    return cost
        ...
        if xtalkG.nodes[qcq].get('frequency', False):
            if sum(qcq[0]) % 2:
                ...
                T2Cost2 = twoq_T2_err(xtalkG.nodes[qcq]['frequency'], chip.nodes[qcq[1]]['frequency'], chip.nodes[qcq[1]]['sweet point'],
                                    a[12], xtalkG.nodes[qcq]['two tq'], chip.nodes[qcq[1]]['df/dphi'])
            if T1Cost1 / a[11] > 0.004 or T1Cost2 / a[11] > 0.004:
                if not(qcq in reOptimizeQCQs):
                    ...
                    print(qcq, 'bad T1 freq', T1Cost1 / a[11], T1Cost2 / a[11])
            if T2Cost1 / a[12] > 400 or T2Cost2 / a[12] > 400:
                if not(qcq in reOptimizeQCQs):
                    if nx.has_path(xtalkG, centertwoQ, qcq):
                        reOptimizeQCQs[qcq] = nx.shortest_path_length(xtalkG, centertwoQ, qcq)
                    else:
                        reOptimizeQCQs[qcq] = 100000
                    print(qcq, 'bad T2 freq', T2Cost1 / a[12], T2Cost2 / a[12])
            for q in qcq:
                for neighbor in chip[q]:
                    if neighbor in qcq:
                        continue
...
a = [1, 0.01, 100, 0.1, 100, 0.1, 6, 100, 0.04, 100, 0.04, 
        0.1, 0.1, 10, 0.04, 10, 0.04, 10, 0.04, 10, 0.04]
...
for _ in range(30):
    ...
    cost = 100000
    for _ in range(10):
        res = minimize(cost_function, ini_frequency, args=(chip, reOptimizeNodes, a),
        ...
        if round(float(res.fun), 3) == 0.0:
            ...
            ini_frequency = [i for i in res.x]
            result = res.x
...
for level in range(len(maxParallelCZs)):
    ...
    for _ in range(40):
        ...
        cost = np.inf
        for _ in range(1):
            res = minimize(twoQ_cost_function, ini_frequency, args=(chip, xTalkSubG, reOptimizeQCQs, a),

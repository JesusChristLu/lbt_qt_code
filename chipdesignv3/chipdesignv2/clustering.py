...
class Clustering():
    ...
    def community_detection(self):
        ...
        while not atoms:
            ...
            if maxNodeCentrality > maxEdgeCentrality:
                if maxNode in maxEdge:
                    if abs(nodesCentrality[maxEdge[0]] - nodesCentrality[maxEdge[1]]) > nodesCentrality[maxNode]:############
                        ...
                        for node in paths: # calculate the pair betweenness by search all the shortest paths
                        ...
                        for k in range(len(neighbors) - 2):
                            ...
                            contract = [np.where(pairBetweennessMat == np.min(pairBetweennessMat))[0][0], # combine the vertexes with least pair betweenness
            ...
            if number_connected_components(G) > componentsNumber:
                ...
                cluster = {}
                community_rank = 0
                for node in G.nodes: # set the community label to the nodes in G
        ...
        for i in modularity:
            ...
            if i > 1 and modularity.index(i) < 0.5 * len(modularity):

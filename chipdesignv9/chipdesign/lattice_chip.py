...
class Lattice():
    def __init__(self, n, mat, show=False):
        if n < 4:
            n = 4
        self.show = show
        ...
        self.square_lattice = self.square_chip(self.height, self.width)
        self.cross_square_lattice = self.cross_square_chip(self.height, self.width)
    ...
    def square_chip(self, h, w):
        graph = nx.grid_2d_graph(h, w)
        graph = nx.relabel.relabel_nodes(graph, self.coordinate2node)
        return graph

    def cross_square_chip(self, h, w):
        graph = nx.grid_2d_graph(h, w)
        for i in range(h - 1):
            for j in range(w // 2):
                if i % 2:
    ...
    def map(self, h, w):
        c2n = dict([((i, j), w * i + j) for i in range(h) for j in range(w)])
    ...
    def liGuShu(self):
        ...
        if self.show:
            nx.draw(busAddLattice, with_labels=True, font_color='white')          
            plt.show()

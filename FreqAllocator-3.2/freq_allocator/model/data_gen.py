...
class MyDataset(Dataset):
    ...
    def __init__(self, H, W, chip, data=None, labels=None, ifFake=False, qid=0):
        ...
        if ifFake:
            ...
            indices = (((data - minFreq * 1e-3) * 1e3)) / (maxFreq - minFreq)
            # indices = (((data - minFreq * 1e-3)))

            # 创建一个零初始化的 B tensor
            x = torch.zeros(data.size()[0] * data.size()[1], data.size()[1] + 1)

            # 将 A tensor 中的值复制到 x tensor 中对应位置
            for i in range(data.size()[1]):
                x[data.size()[0] * i : data.size()[0] * (i + 1), :data.size()[1]] = indices
                x[data.size()[0] * i : data.size()[0] * (i + 1), -1] = i
            ...
            # x = (data - data[:, qid].view(-1, 1))
            # x = torch.cat((x[:, :qid], x[:, qid + 1:]), dim=1)

            # y = torch.tensor(labels)[:, qid]
            # maxErr = torch.max(y)
            # minErr = torch.min(y)
            # y = (y - minErr) / (maxErr - minErr)

...
import excel "F:\BaiduNetdiskWorkspace\vs experiment\finance\combineRussia.xls", sheet("Sheet1") firstrow
save "F:\BaiduNetdiskWorkspace\vs experiment\finance\combineRussia.dta"
use "F:\BaiduNetdiskWorkspace\vs experiment\finance\combineRussia.dta"

import excel "F:\BaiduNetdiskWorkspace\vs experiment\finance\russiaff.xls", sheet("Sheet1") firstrow
save "F:\BaiduNetdiskWorkspace\vs experiment\finance\russiaff.dta"
use "F:\BaiduNetdiskWorkspace\vs experiment\finance\russiaff.dta"

merge m:1 date using "F:\BaiduNetdiskWorkspace\vs experiment\finance\russiaff.dta"
drop if _merge == 2
drop _merge
...
display `i'
*得到估计窗口中的正常回报率
reg ret_rf MktRF SMB HML if event_estimate==1 & id==`i'
*估计在事件窗口内计算正常回报率
predict p if id==`i'
...
*计算超额收益AR
gen AR=ret_rf-predict_return if event_window==1 
...
gen test=(1/sqrt(127))*(CAR/ar_sd) //生成test统计量

...
if __name__ == '__main__':
    ...
    anharm = {'q0' : -240 * 1e-3, 'c0' : -200 * 1e-3, 'q1' : -200 * 1e-3, 'c1' : -200 * 1e-3, 'q2' : -180 * 1e-3}
    C_q = [97, 99, 97]
    C_c = [95, 100]
    ...
    err_list = p.starmap(spectator_CZ, zip(gatetimes, g_qcs, g_qqs, freqqMaxs, freqcMaxs,
                                            bitNums, energyLevels, anharms, sq2, lambdas))
    p.close()
    p.join()
    t = time.time() - tStart
    print(t)
    ...
    err0Picname = 'czerr0' + str(nn)
    err1Picname = 'czerr1' + str(nn)

...
class QuantumGNN(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.dropout(F.relu(self.fc2(x)), train=True, p=0.05) + x
        x = torch.dropout(F.relu(self.fc3(x)), train=True, p=0.05) + x
        x = torch.dropout(F.relu(self.fc7(x)), train=True, p=0)
        x = torch.dropout(F.relu(self.fc8(x)), train=True, p=0) + x
    ...
    def train_model(self, model, data_loader, minMaxErr):
        ...
        for epoch in range(epoches):
            ...
            if (not(epoch % 10) or loss < 1e-4) and not(a_i is None):
    ...
    def test_model(self, model, data_loader, minMaxErr):
        ...
        for batch_data in data_loader:
            ...
            if a_i is None and loss < 1.5e-3:
                a_i = output.squeeze().cpu().detach()
                b_i = batch_data[1].to(self.device).cpu()
            elif loss < 1.5e-3:
                a_i = torch.cat([a_i, output.squeeze().cpu().detach()])

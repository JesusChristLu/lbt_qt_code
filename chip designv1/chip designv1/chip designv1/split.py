...
class Split():
    def __init__(self, g, prog, recover, mat):
        self.graph = deepcopy(g)
        self.prog = prog
        self.recover = recover
        self.mat = mat
        self.splitVertex = self.get_split_vertex()
        self.graph, self.layout = self.split_vertex(self.splitVertex, recover)

    def get_split_vertex(self):
        splitVertex = list(np.where(np.array(list(dict(self.graph.degree(self.graph.nodes)).values())) > Prune.biggestDeg)[0])
        return splitVertex

    def split_vertex(self, splitVertexes, recover):
        ...
        for v in splitVertexes:
            splitMethod = self.choose_media_structure(v, graph, recover)
            I1 = splitMethod[2]
            I2 = splitMethod[3]
            ...
            for node in splitMethod[0]:
                mapping[node] = node + max(list(dict(graph.nodes).keys())) + 1
        ...
        while len(self.recover) > 0:
            ...
            is_planar = check_planarity(graph)

            is_break = False
            if max(list(dict(graph.degree).values())) > Prune.biggestDeg:
                is_break = True

            if not is_planar[0] or is_break:
    ...
    def get_media_structure(self, n, k):
        ...
        if (Prune.biggestDeg - 1) * n - k <= 0:
            return False
        for i in range(1, min((Prune.biggestDeg - 1) * n - k + 2, int(n * (n - 1) / 2) - n + 2)):
            structures = []
    ...
    def choose_media_structure(self, v, graph, recover):
        I, node2index, index2node, I1, I2 = self.get_I(self.prog, v, graph, recover)
        k = graph.degree[v]
        n = 2
        scoresN = []
        mediaStructures = {}
    ...
    def weight_allocation(self, graph, v, mediaStructure, I, node2index, index2node, D, neighbors, neighborNum):
        ...
        while True:
            ...
            if max(list(dict(mediaStructure.degree(list(allocation.keys()))).values())) > Prune.biggestDeg or not is_planar:
        ...
        while len(neighbors) > 0:
            ...
            for n in allocation:
                if mediaStructure.nodes[n]['branch']:
                    continue
                if mediaStructure.degree[n] + 1 > Prune.biggestDeg:
    ...
    def get_I(self, prog, v, graph, recover):
        neighbors = list(dict(graph.adj[v]).keys())

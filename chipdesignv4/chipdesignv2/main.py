...
if __name__=="__main__":
    ...
    #for file_name in path_list[path_list.index('cm42a_207.qasm'):]:
    for file_name in [path_list[path_list.index('square_root_7.qasm')]]:
        print(file_name)
        ...
        prune = Prune(graph)
        vertexRanking = prune.vertexRanking
        if prune.needPrune:
            split = Split(graph, edges, mat, vertexRanking, show)
            algChip = split.graph
